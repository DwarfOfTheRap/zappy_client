    ...
    (loop
              ...
              ((cl-ppcre:scan "^(ok)|(ko)$" str)
               (progn
                 (and (or (string= "inventaire" (car command)) (string= "voir" (car command)))
                      (progn (setf command '()) (funcall (car state) 're-sync)))
                 ...
                 (and (string= "ko" str) (string= (car command) "pre" :end1 3)
                      (or (and (funcall (cdr state) 'wandering) (funcall (car state) 'redirection))
                          (and (funcall (cdr state) 'hatching) (funcall (car state) 'redirect-hatch)))
                      )
      ...
      (if (null command)
          (cond
            ...
            ((funcall (cdr state) 'redirect-hatch)
             (progn
               (set-and-send command '("droite") socket)
               (setf vision nil)
               (funcall (car state) 'hatching))
             )
            ...
            ((funcall (cdr state) 're-sync)
             (progn (funcall (car state) 'chill)
                    (setf command nil)
                    (set-and-send command '("connect_nbr") socket) 
                    )
             )
            ...
            ((funcall (cdr state) 'stopping)
             (progn (if (> (cdar inventory) 11)
                      (set-and-send command '("pose nourriture" "inventaire") socket)
                      (set-and-send command (append (make-path (car (search-in-vision '(|nourriture|) vision))
            ...
            ((funcall (cdr state) 'wait-for-hatch)
               ...
               (if (and (not (member '|nourriture| needs)) stock)
                   ...
                   (if (null needs)
                       (progn (set-and-send command (cons (format nil "broadcast ~a, ~a" team level) nil) socket)
                              (setf clock 0) (funcall (third present) 0) (funcall (third counter) 0))
