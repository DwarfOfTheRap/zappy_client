...
(define-test
    ...
    (let ((state (set-state)) (counter (set-counter)))
      (assert-false (get-broadcast "message 6,youpitralala" "team" 3 nil state nil nil nil))
      (assert-false (get-broadcast "message 6,team, 4" "team" 3 nil state nil nil nil))
      (assert-false (get-broadcast "message 6,team, 3" "team" 5 nil state nil nil nil))
      (assert-false (get-broadcast "message 6,team, 3" "teamal" 3 nil state nil nil nil))
      (assert-false (get-broadcast "message 6,teamo, 3" "team" 3 nil state nil nil nil))
      (assert-true (funcall (cdr state) 'wandering))
      (assert-equal '(6 . elevation) (get-broadcast "message 6,team, 3" "team" 3 nil state nil nil nil))
      (assert-true (funcall (cdr state) 'respond))
      (funcall (car state) 'broadcasting)
      ( get-broadcast "message 4,ready: team" "team" 3 counter state nil nil nil)
      (assert-true (= 0 (funcall (fourth counter))))
      ( get-broadcast "message 0,ready: team" "team" 3 counter state nil nil nil)
...
(define-test pathing-test
    (let ((count (set-counter)))
      (assert-equal '("avance" "avance" "avance" "droite" "avance" "avance" "prend linemate")
                    (make-path '(|linemate| . 13) count))
      (assert-equal '("prend linemate") (make-path '(|linemate| . 0) count))
      (assert-equal '("avance" "prend linemate") (make-path '(|linemate| . 1) count))
      )
  (let ((state (set-state)))
    (assert-equal '("gauche") (join-for-incantation 4 nil "test" state 4))
    (assert-equal '("droite") (join-for-incantation 6 nil "test" state 4))
    (assert-equal '("avance" "avance" "gauche" "avance" "avance" "inventaire") (join-for-incantation 2 nil "test" state 1))
    (assert-equal '("broadcast ready: test") (join-for-incantation 0 nil "test" state 4))
    )
  )
...
(define-test test-all
    (let ((count (set-counter)))
      (assert-equal '("avance" "avance" "gauche" "avance" "avance" "prend nourriture")
                    (make-path (car (search-in-vision (check-inventory *inventory01* 1) *vision03*)) count))
      (assert-equal '("gauche" "avance")
                    (make-path (car (search-in-vision (check-inventory *inventory01* 1) *vision04*)) count))
      (assert-equal '("avance" "prend phiras")
                    (make-path (car (search-in-vision (check-inventory *inventory02* 3) *vision04*)) count))
      )
  )
