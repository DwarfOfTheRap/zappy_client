...
(define-test
    ...
    (let ((state (set-state)) (counter (set-counter)))
      (assert-false (get-broadcast "message 6,youpitralala" "team" 3 nil state nil nil))
      (assert-false (get-broadcast "message 6,team, 4" "team" 3 nil state nil nil))
      (assert-false (get-broadcast "message 6,team, 3" "team" 5 nil state nil nil))
      (assert-false (get-broadcast "message 6,team, 3" "teamal" 3 nil state nil nil))
      (assert-false (get-broadcast "message 6,teamo, 3" "team" 3 nil state nil nil))
      (assert-true (funcall (cdr state) 'wandering))
      (assert-equal '(6 . elevation) (get-broadcast "message 6,team, 3" "team" 3 nil state nil nil))
      (assert-true (funcall (cdr state) 'respond))
      (funcall (car state) 'broadcasting)
      ( get-broadcast "message 4,ready: team" "team" 3 counter state nil nil)
      (assert-true (= 0 (funcall (fourth counter))))
      ( get-broadcast "message 0,ready: team" "team" 3 counter state nil nil)
