...
(define-test
    ...
    (let ((state (set-state)) (counter (set-counter)))
      ...
      (assert-false (get-broadcast "message 6,team, 3" "teamal" 3 nil state nil))
      (assert-false (get-broadcast "message 6,teamo, 3" "team" 3 nil state nil))
      (assert-true (funcall (cdr state) 'wandering))
      (assert-equal '(6 . elevation) (get-broadcast "message 6,team, 3" "team" 3 nil state nil))
...
(define-test pathing-test
  ...
  (let ((state (set-state)))
    (assert-equal '("gauche" "inventaire") (join-for-incantation 4 nil "test" state 4))
    (assert-equal '("droite" "inventaire") (join-for-incantation 6 nil "test" state 4))
